            ...
            <tbody className="divide-y divide-slate-800/60">
              {orders.map((ord) => (
                <tr key={ord.id} className="hover:bg-[#f6fbf7]/40">
                  <td data-label="Order ref" className="py-4 px-6 font-mono font-bold text-teal-700">{ord.id}</td>
                  <td data-label="Produce & volume" className="py-4 px-6 font-semibold text-[#163025]">{ord.qtyKg} kg {ord.produce}</td>
                  <td data-label="Total cost" className="py-4 px-6 font-extrabold text-emerald-700">₱{ord.total.toLocaleString()}</td>
                  <td data-label="Date" className="py-4 px-6 text-xs text-[#5f7469]">{ord.date}</td>
                  ...
                  <td data-label="Details" className="py-4 px-6 text-right">
                    <Link
                      href={`/buyer/orders/${ord.id}`}
                      className="min-h-11 px-3 py-1.5 rounded-xl bg-[#ecfdf5] hover:bg-[#d1fae5] text-[#047857] text-xs font-bold inline-flex items-center gap-1"
                    >
